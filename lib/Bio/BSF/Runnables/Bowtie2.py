...
from Bio.BSF.Argument import OptionShort
from Bio.BSF.Executables import Bowtie2
...
def run_picard_sam_to_fastq(runnable, bam_file_path):
    ...
    java_process.add_OptionShort(key='jar', value=os.path.join(default.classpath_picard, 'SamToFastq.jar'))
    java_process.add_SwitchShort(key='Xmx2G')
    java_process.add_OptionPair(key='-Djava.io.tmpdir', value=runnable.file_path_dict['temporary_directory'])

    sam_to_fastq = java_process.sub_command
    sam_to_fastq.add_OptionPair(key='INPUT', value=bam_file_path)
    sam_to_fastq.add_OptionPair(key='OUTPUT_PER_RG', value='true')
    sam_to_fastq.add_OptionPair(key='OUTPUT_DIR', value=runnable.file_path_dict['temporary_directory'])
    sam_to_fastq.add_OptionPair(key='INCLUDE_NON_PF_READS', value='false')  # TODO: Make this configurable.
    sam_to_fastq.add_OptionPair(key='TMP_DIR', value=runnable.file_path_dict['temporary_directory'])
...
def run_bowtie2(runnable):
    ...
    assert isinstance(bowtie2, Bio.BSF.Executables.Bowtie2)
    ...
    for rgc in rgc_list:
        ...
        if rgc.fastq_1_path and rgc.fastq_2_path:
            bowtie2.add_OptionShort(key='1', value=rgc.fastq_1_path)
            bowtie2.add_OptionShort(key='2', value=rgc.fastq_2_path)
        elif rgc.fastq_1_path:
            bowtie2.add_OptionShort(key='U', value=rgc.fastq_1_path)
    ...
    if len(fastq_list):
        ...
        bowtie2.add_OptionShort(key='U', value=string.join(words=fastq_list, sep=','))
