...
class ChIPSeqDiffBindSheet(AnnotationSheet):
    """ChIP-Seq Bioconductor DiffBind annotation sheet.
    """
    ...
    test_methods = dict(
        SampleID=AnnotationSheet.check_alphanumeric,
...
class ChIPSeq(Analysis):
    ...
    def _create_bwa_jobs(self):
        ...
        alignment_drms = DRMS.from_Analysis(
            name='chipseq_alignment',
            work_directory=self.genome_directory,
            analysis=self)
        self.drms_list.append(alignment_drms)
        ...
        for sample in self.samples:
            ...
            for replicate_key in replicate_keys:
                ...
                bwa_mem.add_OptionShort(key='t', value=str(alignment_drms.threads))
                ...
                run_bwa = Executable.from_Analysis(
                    ...
                    analysis=self)
                alignment_drms.add_Executable(executable=run_bwa)
    ...
    def _create_bowtie2_jobs(self):
        ...
        # bowtie2_drms = DRMS.from_Analysis(
        ...
        # sam2bam_drms = DRMS.from_Analysis(
        #     name='sam2bam',
        #     work_directory=self.genome_directory,
        #     analysis=self)
        ...
        for sample in self.samples:
            ...
            for replicate_key in replicate_keys:
                ...
                runnable = Runnable(
                    name=prefix,
                    code_module='Bio.BSF.Runnables.Bowtie2',
                ...
                bowtie2 = Bowtie2(
                    name='bowtie2',
                    analysis=self)
                runnable.add_executable(executable=bowtie2)

                # Set Bowtie2 options.

                bowtie2.add_OptionShort(key='x', value=bowtie2_index)
                bowtie2.add_OptionLong(key='threads', value=str(alignment_drms.threads))
                ...
                java_process = Executable(name='picard_clean_sam', program='java', sub_command=Command(command=str()))
                ...
                clean_sam.add_OptionPair(key='INPUT', value=file_path_dict['aligned_sam'])
                clean_sam.add_OptionPair(key='OUTPUT', value=file_path_dict['cleaned_sam'])
                clean_sam.add_OptionPair(key='TMP_DIR', value=file_path_dict['temporary_directory'])
                ...
                java_process = Executable(name='picard_sort_sam', program='java', sub_command=Command(command=str()))
                runnable.add_executable(executable=java_process)
                ...
                sort_sam.add_OptionPair(key='INPUT', value=file_path_dict['cleaned_sam'])
                sort_sam.add_OptionPair(key='OUTPUT', value=file_path_dict['sorted_bam'])
                sort_sam.add_OptionPair(key='SORT_ORDER', value='coordinate')
                sort_sam.add_OptionPair(key='TMP_DIR', value=file_path_dict['temporary_directory'])
                ...
                executable = Executable.from_analysis_runnable(analysis=self, runnable_name=prefix)
    ...
    def _create_macs14_jobs(self):
        ...
        macs14_drms = DRMS.from_Analysis(
            name='macs14',
            work_directory=self.genome_directory,
            analysis=self)
        self.drms_list.append(macs14_drms)

        process_macs14_drms = DRMS.from_Analysis(
            name='process_macs14',
            work_directory=self.genome_directory,
            analysis=self)
    ...
    def _create_macs2_jobs(self):
        ...
        peakcalling_drms = DRMS.from_Analysis(
                ...
                for t_replicate_key in t_replicate_keys:
                    ...
                    for c_sample in chipseq_comparison.c_samples:
                        ...
                        for c_replicate_key in c_replicate_keys:
                            ...
                            runnable = Runnable(
                                name=prefix,
                                code_module='Bio.BSF.Runnables.ChIPSeqPeakCalling',
                                working_directory=self.genome_directory,
                                file_path_dict=file_path_dict)
                            self.add_runnable(runnable=runnable)
                            ...
                            macs2_callpeak = Macs2Callpeak(
                                name='macs2_callpeak',
                                analysis=self)
                            runnable.add_executable(executable=macs2_callpeak)
                            ...
                            macs2_bdgcmp = Macs2Bdgcmp(
                                ...
                                analysis=self)
                            runnable.add_executable(executable=macs2_bdgcmp)
                            ...
                            process_macs2 = Executable.from_Analysis(
                                ...
                                analysis=self)
                            runnable.add_executable(executable=process_macs2)
    ...
    def report(self):
        ...
        for key in keys:
            ...
            sas = AnnotationSheet.read_from_file(file_path=file_path)
