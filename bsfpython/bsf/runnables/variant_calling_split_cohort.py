"""bsf.runnables.variant_calling_split_cohort
...
def run_gatk_select_variants(runnable):
    ...
    if os.path.exists(runnable.file_path_dict['sample_vcf']):
        return

    runnable.run_executable(name='gatk_select_variants')
...
def run_gatk_variants_to_table(runnable):
    ...
    if os.path.exists(runnable.file_path_dict['sample_tsv']):
        return

    run_gatk_select_variants(runnable=runnable)
    runnable.run_executable(name='gatk_variants_to_table')
